...
template <int NT_producer, int NT_Consumer, int Stride=1>
using PPTScheduler = SimpleDynamicScheduler<NT_producer, NT_Consumer, Stride>;
...
__global__ void SimpleProducerConsumer(
    const int* const __restrict__ src, 
    int* const __restrict__ dst, 
    int array_len
) {
    ...
    PPTScheduler<96, 288, 2> scheduler(num_works, work_cnt_ptr, smem_ptr);
    __syncthreads();
    ...
    if (warp_group_id == 0) {
        if (warp_id > 0) {
            for (int work_id = scheduler.get_initial_work<true>(); scheduler.is_valid(work_id); work_id = scheduler.get_next_work<true>(work_id)) {
                __nanosleep(1000);
                if (threadIdx.x == 96) {
                    DEBUG_PRINT("(%03d/%03d) Producer job: %d, job: %d / %d, skippable: %d\n", blockIdx.x, gridDim.x, counter++, work_id, array_len, int(scheduler.skippable()));
                }
                scheduler.prefetch_next_work(work_id);
            }
            named_barrier_sync(96, static_cast<uint32_t>(Barrier::ProducerSync) /*id*/);
            if (threadIdx.x == 96)
                DEBUG_PRINT("(%03d/%03d) Producer quitted: %d\n", blockIdx.x, gridDim.x, counter);
        } else {
            scheduler.init_consumer();
            for (int work_id = scheduler.get_initial_work<false>(); scheduler.is_valid(work_id); work_id = scheduler.get_next_work<false>(work_id)) {
                __nanosleep(1500);
                if (threadIdx.x == 0) {
                    DEBUG_PRINT("(%03d/%03d) Consumer 1 job: %d, job: %d / %d\n", blockIdx.x, gridDim.x, counter++, work_id, array_len);
                }
            }
            named_barrier_sync(32, static_cast<uint32_t>(Barrier::Consumer1Sync) /*id*/);
            if (threadIdx.x == 0) {
                DEBUG_PRINT("(%03d/%03d) Consumer 1 quitted: %d\n", blockIdx.x, gridDim.x, counter);
            }
        }
        ...
        for (int work_id = scheduler.get_initial_work<false>(); scheduler.is_valid(work_id); work_id = scheduler.get_next_work<false>(work_id)) {
            ...
            if (threadIdx.x == 128) {
                DEBUG_PRINT("(%03d/%03d) Consumer 2 job: %d, job: %d / %d\n", blockIdx.x, gridDim.x, counter++, work_id, array_len);
            }
        }
        named_barrier_sync(256, static_cast<uint32_t>(Barrier::Consumer2Sync) /*id*/);
        if (threadIdx.x == 128)
...
void TensorProcessor::unordered_expand(const DLManagedTensor* in_tensor, DLManagedTensor* out_tensor) const {
    ...
    SimpleProducerConsumer<<<num_blocks, 384>>>(
